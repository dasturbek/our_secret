...
main_application.title('Bekpad')
main_application.wm_iconbitmap('dasturbek.ico')
...
##text input
keyinput = ttk.Entry(tool_bar, width=20, font=('Arial 18'))
keyinput.grid(row=1, column=0, columnspan=2, padx=5)
def shefr():
    key = keyinput.get()
    ...
    if len(key) != 0:
        if len(key) > len(text):
            key = key[0:(len(text)-1)]
        for i in range(len(text)):
            shefr += str(chr((ord(text[i]) + ord(key[i % len(key)])) % 256))
        text_editor.delete(1.0, "end")
        text_editor.insert("end", shefr)
...
##shefr
shefr_icon = tk.PhotoImage(file='icons2/shefr.png')
shefr_btn= ttk.Button(tool_bar,image=shefr_icon)
shefr_btn['command'] = lambda: shefr()
shefr_btn.grid(row=1,column=2,padx=5)

##deshefr
deshefr_icon = tk.PhotoImage(file='icons2/deshefr.png')
deshefr_btn=ttk.Button(tool_bar,image=deshefr_icon)
deshefr_btn['command'] = lambda: deshefr()
deshefr_btn.grid(row=1,column=3,padx=5)
